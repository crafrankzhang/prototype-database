	...
	if(compOp != NO_OP)
	{
			...
			while(true)
			{
				...
				while(slotnum >=scanid.slotNum)
				{
				 ...
				 int length;
				 int startpos;
				 memcpy(&startpos, (byte*)page + PAGE_SIZE - scanid.slotNum * 2 * sizeof(unsigned  int) ,sizeof(unsigned  int));

			      memcpy(&length, (byte*)page + PAGE_SIZE - scanid.slotNum * 2 * sizeof(unsigned  int) + sizeof(unsigned  int) ,sizeof(unsigned  int));
